...
try:
    from edge_tts.submaker import mktimestamp  # type: ignore
except ImportError:
    # Fallback for newer edge_tts versions
    def mktimestamp(offset):
        return str(offset)
from loguru import logger  # type: ignore
from moviepy.video.tools import subtitles  # type: ignore
...
def get_qwen_voices() -> list[str]:
    ...
    获取Qwen3 TTS的声音列表
    """
    return ["qwen:Aiden-Male"]


def get_gemini_voices(model_prefix: str) -> list[str]:
    """
    获取 Gemini TTS 的声音列表
    """
    voices = [
    ...
    return [f"{model_prefix}:{v}" for v in voices]
...
def tts(
    ...
    if is_azure_v2_voice(voice_name):
        ...
        # Qwen3 TTS logic
        parts = voice_name.split(":")
        voice = parts[1].split("-")[0] if len(parts) > 1 else "Aiden"
        return qwen_tts(text, voice, voice_rate, voice_file)
    elif is_gemini_voice(voice_name):
        # Gemini TTS logic
        parts = voice_name.split(":")
        model_prefix = parts[0]
        voice = parts[1].split("-")[0] if len(parts) > 1 else "Puck"
        return gemini_tts(text, model_prefix, voice, voice_file)
...
def siliconflow_tts(
    ...
    # 确保gain在[-10, 10]范围内
    gain = max(-10.0, min(10.0, float(gain)))  # type: ignore
                ...
                try:
                    ...
                    if sentences:
                        ...
                        for sentence in sentences:
                            ...
                            curr_duration = int(sentence_chars * char_duration)  # type: ignore

                            # 添加到SubMaker
                            sub_maker.subs.append(sentence)
                            start_offset = int(current_offset)  # type: ignore
                            end_offset = start_offset + curr_duration
                            sub_maker.offset.append((start_offset, end_offset))

                            # 更新偏移量
                            current_offset = int(end_offset)  # type: ignore
                    else:
...
def qwen_tts(
    ...
    """
    Generate speech using a Qwen3 Colab endpoint via Pinggy SSH tunnel.
    Includes strict response validation, sentence chunking, and ffprobe duration.
    """
    import re
    import subprocess
    import os
    ...
    text = text.strip()
    pinggy_url = config.qwen.get("pinggy_url", "").strip("/")
    
    if not pinggy_url:
        logger.error("Qwen3 TTS Pinggy URL is not set in config")
        return None
        
    url = f"{pinggy_url}/tts"
    
    # Required headers – bypass Pinggy / Localtunnel / ngrok interstitial pages
    headers = {
    ...
    # --- Chunk long text by sentences to avoid T4 GPU OOM ---
    MAX_CHUNK_CHARS = 500
    ...
    if isinstance(current_chunk, str) and current_chunk.strip():
        chunks.append(current_chunk.strip())

    if not chunks:
        chunks = [text]

    logger.info(f"Qwen TTS: split text into {len(chunks)} chunk(s)")

    all_audio_bytes: bytes = b""
    ...
    for chunk_idx, chunk_text in enumerate(chunks):
        ...
        for i in range(3):
            try:
                logger.info(f"start qwen tts, voice: {voice}, url: {url}, chunk: {chunk_idx + 1}/{len(chunks)}, try: {i + 1}")
                response = requests.post(url, json=payload, headers=headers, timeout=120)

                # 1. Enforce status codes
                response.raise_for_status()

                # 2. Validate MimeType (reject HTML/JSON error pages)
                content_type = response.headers.get('Content-Type', '')
                if 'text/html' in content_type or 'application/json' in content_type:
                    raise ValueError(
                        f"API returned {content_type} instead of audio. "
                        f"Tunnel blocked or API crashed! Response snippet: {response.text[:200]}"
                    )

                # 3. Verify we got actual audio bytes (not an empty or tiny response)
                if len(response.content) < 1000:
                    raise ValueError(
                        f"API returned suspiciously small response ({len(response.content)} bytes). "
                        f"Snippet: {response.content[:200]}"
                    )

                all_audio_bytes += bytes(response.content)  # type: ignore[arg-type]
                chunk_success = True
                logger.info(f"Qwen TTS chunk {chunk_idx + 1}/{len(chunks)} received: {len(response.content)} bytes")
                break  # success, move to next chunk

            except Exception as e:
                logger.error(f"qwen tts chunk {chunk_idx + 1} failed on try {i + 1}: {repr(e)}")

        if not chunk_success:
            logger.error(f"qwen tts: all retries failed for chunk {chunk_idx + 1}. Aborting.")
            return None

    # --- Write concatenated audio bytes to disk ---
    with open(voice_file, "wb") as f:
        f.write(all_audio_bytes)

    logger.info(f"Qwen TTS: wrote {len(all_audio_bytes)} bytes to {voice_file}")

    # --- Get duration via ffprobe (zero RAM) ---
    audio_duration = 0.0
    try:
        ...
        if duration_str == 'N/A' or not duration_str:
            with open(voice_file, 'r', encoding='utf-8', errors='ignore') as f:
                snippet = f.read(300)
            raise ValueError(
                f"FFprobe returned N/A. File is not valid audio! "
                f"Content snippet: {snippet}"
            )
        audio_duration = float(duration_str)
    except Exception as e:
        logger.error(f"FFprobe failed to read qwen audio duration: {repr(e)}")
        return None

    if audio_duration <= 0:
        logger.error(f"Qwen TTS: ffprobe returned invalid duration ({audio_duration}). File may be corrupt.")
        return None
    ...
    try:
        audio_duration_100ns = int(audio_duration * 10000000)
        sub_sentences = utils.split_string_by_punctuations(text)
        ...
        logger.warning(f"Failed to create accurate qwen subtitles: {str(e)}")
        sub_maker.subs = [text]
        sub_maker.offset = [(0, int(audio_duration * 10000000))]
                
    logger.success(f"qwen tts succeeded: {voice_file} (duration: {audio_duration:.2f}s)")
    return sub_maker
            ...
            if len(sentence) > 120:
                ...
                if len(parts) > 1:
                    ...
                    for j, part in enumerate(parts):
                        if j == len(parts) - 1:
                            processed_sentences.append(part + punctuation)  # type: ignore
                        else:
                            processed_sentences.append(part.strip() + '.')  # type: ignore
                else:
                    processed_sentences.append(sentence + punctuation)  # type: ignore
            else:
                processed_sentences.append(sentence + punctuation)  # type: ignore
        else:
            processed_sentences.append(sentences[i])  # type: ignore
...
def chatterbox_tts(
    ...
    logger.info(f"Chatterbox TTS input: '{text[:100]}...' (original: {len(original_text)} → processed: {len(text)} chars)")  # type: ignore
                ...
                if "words" in segment and segment["words"]:
                    for word_info in segment["words"]:
                        ...
                        if word and start is not None and end is not None and start < end:
                            ...
                            word_count = int(word_count) + 1  # type: ignore
                        else:
        ...
        if not sub_maker.subs or transcription_failed:
            ...
            if sentences:
                ...
                for sentence in sentences:
                    ...
                    sentence_chars = len(sentence)
                    curr_duration = int(float(sentence_chars) * float(char_duration))  # type: ignore
                    
                    sub_maker.subs.append(str(sentence).strip())
                    start_offset = int(current_offset)  # type: ignore
                    end_offset = start_offset + curr_duration
                    sub_maker.offset.append((start_offset, end_offset))
                    current_offset = int(end_offset)  # type: ignore
    ...
    try:
        for i, chunk in enumerate(chunks):
            ...
            if chunk_result:
                ...
                cumulative_offset_100ns = int(cumulative_duration * 10000000)  # type: ignore
...
def combine_audio_files(audio_files: list, output_file: str) -> str:
    ...
    """
    try:
        from moviepy import AudioFileClip, concatenate_audioclips  # type: ignore
...
def gemini_tts(text: str, model_prefix: str, voice: str, voice_file: str) -> Union[SubMaker, None]:  # type: ignore
    """
    使用 Google Gemini API 生成语音
    model_prefix takes either 'gemini_flash' or 'gemini_pro'
    """
    ...
    if model_prefix == "gemini_flash":
        model_name = "gemini-2.5-flash-preview-tts"
    elif model_prefix == "gemini_pro":
        model_name = "gemini-2.5-pro-preview-tts"
    else:
        logger.error(f"Invalid Gemini TTS model prefix: {model_prefix}")
        return None  # type: ignore

    url = f"https://generativelanguage.googleapis.com/v1beta/models/{model_name}:generateContent?key={api_key}"

    # As per Google Generative Language generateContent schema for TTS
    payload = {
        "contents": [
            {
                "parts": [{"text": text}]
    ...
    for i in range(3):
        try:
            logger.info(f"start gemini tts (REST API), model: {model_name}, voice: {voice}, try: {i + 1}")
            response = requests.post(url, json=payload, headers=headers)  # type: ignore

            if response.status_code == 200:
                resp_json = response.json()
                
                # Parse generateContent format
                candidates = resp_json.get("candidates", [])
                if not candidates:
                    logger.error(f"Gemini TTS failed: No candidates in response. Full struct: {resp_json}")
                    return None  # type: ignore
                    
                parts = candidates[0].get("content", {}).get("parts", [])
                if not parts:
                    logger.error(f"Gemini TTS failed: No parts in response.")
                    return None  # type: ignore
                ...
                if not audio_base64:
                    logger.error(f"Gemini TTS failed: Empty audio string in inlineData. Full struct: {resp_json}")
                    return None  # type: ignore
                ...
                def get_audio_duration(file_path):
                    try:
                        ...
                        logger.error(f"FFprobe failed to read duration: {e}")
                        return 0.0

                if "mp3" in mime_type or "mpeg" in mime_type:
                    # Native MP3, save directly
                    with open(voice_file, "wb") as f:
                        f.write(audio_data)
                    ...
                    try:
                        ...
                        if audio_duration == 0.0:
                            logger.error(f"Gemini native MP3 duration read failed via ffprobe")
                            return None  # type: ignore
                    except Exception as e:
                        logger.error(f"Gemini native MP3 duration read failed: {str(e)}")
                        return None  # type: ignore
                else:
                    # Parse extension
                    ext = "wav"
                    ...
                    temp_voice_file = f"{voice_file}.{ext}"
                    ...
                    try:
                        ...
                        logger.info(f"Temp file size: {os.path.getsize(temp_voice_file)} bytes")
                        
                        # Convert temp file to MP3 natively via FFMPEG, overwriting if exists (-y)
                        subprocess.run(
                            ['ffmpeg', '-y', '-i', temp_voice_file, voice_file],
                            check=True,
                            capture_output=True,
                            text=True
                        )
                        ...
                        if not os.path.exists(voice_file) or os.path.getsize(voice_file) == 0:
                            raise FileNotFoundError(f"FFmpeg finished, but output file {voice_file} is missing or 0 bytes!")
                        ...
                        import traceback
                        logger.error(f"FFmpeg execution or file handling failed: {repr(e)}\n{traceback.format_exc()}")
                        if os.path.exists(temp_voice_file):
                            os.remove(temp_voice_file)
                        raise e
                    ...
                    try:
                        audio_duration = get_audio_duration(voice_file)
                        if audio_duration == 0.0:
                            logger.error(f"FFprobe failed to read final MP3 duration")
                            if os.path.exists(temp_voice_file):
                                os.remove(temp_voice_file)
                            return None  # type: ignore
                    except Exception as e:
                        import traceback
                        logger.error(f"FFprobe failed to read final MP3: {repr(e)}\n{traceback.format_exc()}")
                        if os.path.exists(temp_voice_file):
                            os.remove(temp_voice_file)
                        return None  # type: ignore
                ...
                sub_maker = ensure_submaker_compatibility(SubMaker())  # type: ignore

                try:
                    audio_duration_100ns = int(audio_duration * 10000000)
                    sentences = utils.split_string_by_punctuations(text)  # type: ignore
                    ...
                    if sentences:
                        ...
                        current_offset = 0
                        for sentence in sentences:
                            if not sentence.strip():  # type: ignore
                                continue
                            sentence_chars = len(sentence)
                            sentence_duration = int(sentence_chars * char_duration)  # type: ignore
                            sub_maker.subs.append(sentence)
                    ...
                    logger.warning(f"Failed to create accurate gemini subtitles: {str(e)}")
                    sub_maker.subs = [text]
                    sub_maker.offset = [(0, audio_duration_100ns if "audio_duration_100ns" in locals() else 10000000)]

                logger.success(f"gemini tts succeeded: {voice_file}")
                return sub_maker
            else:
                logger.error(
                    f"gemini tts failed, status code: {response.status_code}, detail: {response.text}"
                )
                if response.status_code in [401, 403]:
                    logger.error("Please check your Gemini API key")
                    break
        except Exception as e:
                ...
                if is_punctuation_end or is_long_phrase or is_last_word or (is_comma_pause and len(current_phrase) >= 4):
                    if current_phrase:
                        ...
                        subtitle_entry = formatter(
                            idx=subtitle_index,
                            start_time=current_start_time,  # type: ignore
                            end_time=current_end_time,
                            sub_text=phrase_text
                        )
                        subtitle_entries.append(subtitle_entry)
                        subtitle_index += 1  # type: ignore
