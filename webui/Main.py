...
from app.models.schema import (  # type: ignore
    MaterialInfo,
    VideoAspect,
    VideoConcatMode,
    VideoParams,
...
from app.services import llm, voice  # type: ignore
from app.services import task as tm  # type: ignore
...
with middle_panel:
    with st.container(border=True):
        ...
        video_sources = [
            (tr("Pexels"), "pexels"),
            (tr("Pixabay"), "pixabay"),
            (tr("Wikimedia Commons"), "wikimedia"),
            (tr("Local file"), "local"),
            (tr("TikTok"), "douyin"),
        ...
        if params.video_concat_mode.value == "semantic":
            with st.container(border=True):
                ...
                if image_sim_available:
                    ...
                    missing_deps = ", ".join(image_sim_info.get("dependencies", []))  # type: ignore
    ...
    with st.container(border=True):
        ...
        tts_servers = [
            ...
            ("qwen", "Qwen3 TTS (Colab)"),
            ("gemini_flash", "Gemini 2.5 Flash TTS"),
            ("gemini_pro", "Gemini 2.5 Pro TTS"),
        ]
        ...
        elif selected_tts_server.startswith("gemini_"):
            # 获取Gemini声音列表
            filtered_voices = voice.get_gemini_voices(selected_tts_server)
        else:
            # 获取Azure的声音列表
        ...
        # qwen colab configuration
        if selected_tts_server == "qwen" or (
            voice_name and voice.is_qwen_voice(voice_name)
        ):
            saved_qwen_pinggy = config.qwen.get("pinggy_url", "")
            
            qwen_pinggy = st.text_input(
                "Qwen3 Pinggy URL",
                value=saved_qwen_pinggy,
                key="qwen_pinggy_url_input",
                help="The public Pinggy SSH tunnel URL exposing your Google Colab instance, e.g. https://random-string.a.pinggy.link",
            )

            if saved_qwen_pinggy != qwen_pinggy:
                logger.info(
                    f"Qwen Pinggy URL changed: {saved_qwen_pinggy} => {qwen_pinggy}"
                )
                config.qwen["pinggy_url"] = qwen_pinggy
                config.save_config()

        # Google Gemini API configuration
        if selected_tts_server.startswith("gemini_") or (
            voice_name and voice.is_gemini_voice(voice_name)
        ):
            saved_gemini_api_key = config.gemini.get("api_key", "")
            gemini_api_key = st.text_input(
                "Google Gemini API Key",
                value=saved_gemini_api_key,
                type="password",
                key="gemini_api_key_input",
                help="Your API key for Google AI Studio (generativelanguage.googleapis.com). Needed for Gemini 2.5 TTS."
            )

            if saved_gemini_api_key != gemini_api_key:
...
if start_button:
    ...
    if params.video_source not in ["pexels", "pixabay", "local", "wikimedia"]:
    ...
    if not params.voice_name:
        st.error("Please select a valid TTS voice before generating the video.")
        scroll_to_bottom()
        st.stop()
    ...
    def log_received(msg):
        ...
        # 1. Truncate UI Log (keep only the last 100 lines) to fix browser freezing
        if len(log_records) > 100:
            log_records.pop(0)
            
        current_time = time.time()
        # 2. Slow Down Polling: Throttle WebSocket updates to once every 1.5 seconds
        if current_time - last_log_update[0] > 1.5:
            last_log_update[0] = current_time
            with log_container:
                st.code("\n".join(log_records))

    logger_id = logger.add(log_received)
